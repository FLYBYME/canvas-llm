...
export default class Artifact {
        ...
        return this.dynamicDeterminePathFunction()
            .then((res) => {
                if (res === "rewriteArtifact") {
                    return this.rewriteArtifact()
                        .then(() => this.generateFollowup());
                } else if (res === "generateArtifact") {
                    return this.generateNewArtifact()
                        .then(() => this.generateFollowup());
    ...
    async getSuggestedQuestions() {
        ...
        const formattedSuggestedQuestionsPrompt = SUGGESTED_QUESTIONS_PROMPT
            .replace("{overview}", JSON.stringify(await this.generateDescription()))
            .replace("{reflections}", memoriesAsString);
...
${result.content}`;
    ...
    async rewriteArtifact() {
        ...
        const rewriteArtifactResponse = await model.invoke([
            {
                role: "system",
                content: formattedRewriteArtifactPrompt,
            },
            ...this.conversation.formatMessages(),
        ]);
